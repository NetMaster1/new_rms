...
class HI_report(models.Model):
    ...
    def forecast_percent(self):
        if self.HI_plan==0:
            ...
            return int((self.HI/self.date_before_current*self.days_of_the_month)/self.HI_plan*100)
    def forecast_items(self):
        return int(self.HI/self.date_before_current*self.days_of_the_month)
    def average_per_day(self):
        if (self.days_of_the_month-self.date_before_current)==0:
            return int(0)
        else:
            return int((self.HI_plan - self.HI)/(self.days_of_the_month-self.date_before_current))
